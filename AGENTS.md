...
`https://prometheus.bactensor.io`. The validator exposes a minimal `/metrics`
endpoint on port `9080` from `validator/src/validator/metrics.py`. The local
Prometheus scrape config also picks up Pylon's `/metrics` using the Bearer token
from `PYLON_METRICS_TOKEN` (generated by `installer/install.sh.jinja`).

When you extend the validator — new payload creators, scorers, nodes, weight
setters — treat metrics as first-class. Register Prometheus counters, gauges, and
histograms in `validator/src/validator/metrics.py` and instrument the code paths.
Every new subsystem should ship with at least one event counter and one latency
histogram. Keep `validator/README.md` in sync with the metric names you expose.
