...
`bittensor_prometheus` that scrapes the compose network plus the host node-exporter,
and `bittensor-prometheus-proxy` that remote-writes to
`https://prometheus.bactensor.io`. The local Prometheus scrape config also picks
up Pylon's `/metrics` using the Bearer token from `PYLON_METRICS_TOKEN` (generated
by `installer/install.sh.jinja`).

When you extend the validator — new payload creators, scorers, nodes, weight
setters — treat metrics as first-class. The validator itself does not yet expose a
`/metrics` endpoint, and there is no project-specific metrics module to drop
counters into. The right move is to follow Nexus's own conventions: inspect the
...
metrics for its components (nodes, communicators, runtime), and mirror that
approach when adding observability to your validator. Every new subsystem should
ship with at least one event counter and one latency histogram, designed and
named consistently with the Nexus patterns you find there.
